...
def decibel_filter(spectrogram, db_cutoff):
    ...
    in_decibels = 10.0 * np.log10(remove_zeros)
    in_decibels[in_decibels <= db_cutoff] = db_cutoff
    return np.nan_to_num(10.0 ** (in_decibels / 10.0))
...
def open_image(image_bytes):
    ...
    if image.mode not in ("RGBA", "RGB"):
        raise AttributeError(
            "Input image not in RGBA or RGB mode and cannot be processed."
        )
    if image.mode == "RGBA":
...
def open_audio(audio_bytes):
    ...
    # Resample
    print("Resample!")
    samples = resample(samples, input_sample_rate, target_sample_rate, res_type="kaiser_best")

    # Print warning for multi-channel samples
    if samples.ndim > 1:
        samples = to_mono(samples)
        print("rats_detector.py: Warning, multi-channel file detected. Mixed to mono.")
    ...
    # Spectrogram-level manipulation
    print("Manipulation!")
    spectrogram = decibel_filter(spectrogram, -100.0)
...
def generate_detections(detection_graph, image):
    ...
    with tf.Session(graph=detection_graph) as sess:
        ...
        (box, score, clss, num_detections) = sess.run(
            [box, score, clss, num_detections], feed_dict={image_tensor: image_np}
        )
    ...
    return (
        ...
        np.squeeze(clss),
        image,
    )  # these are lists of bboxes, scores etc
...
def render_bounding_boxes(
    boxes, scores, classes, image, label_map={}, confidence_threshold=0.5
):
    ...
    display_strs = (
        []
    )  # list of list, one list of strings for each bounding box (to accommodate multiple labels)
    ...
    draw_bounding_boxes_on_image(
        image, display_boxes, display_str_list_list=display_strs
    )
...
def draw_bounding_boxes_on_image(
    image, boxes, color="LimeGreen", thickness=4, display_str_list_list=()
):
