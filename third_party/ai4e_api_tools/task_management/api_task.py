...
class TaskManager:
    ...
    def UpdateTaskStatus(self, taskId: str, status: Any) -> None:
        with self.task_status_json_lock:
            if not os.path.isfile(self.task_status_json_path):
                raise ValueError(
                    'taskId "{}" is not found. Decorate your endpoint with an ai4e_service decorator or call AddTask(request) before UpdateTaskStatus.'.format(
                        taskId
                    )
                )
    ...
    def AddPipelineTask(self, taskId, organization_moniker, version, api_name, body):
        ...
        if r.status_code != 200:
            ai4e_service.api_task_manager.UpdateTaskStatus(
                taskId,
                "Pipelining is not supported in a single node deployment, but the next service is: "
                + next_url,
            )
            return (
                "Pipelining is not supported in a single node deployment, but the next service is: "
